...
layout(location=3) in vec2 i_Atlas_Size;
layout(location=4) in vec2 i_Rotate_Origin;
layout(location=5) in float i_Rotation;
layout(location=6) in uint i_Atlas_Layer;
layout(location=7) in uint i_Is_Hi_Dpi;
...
void main() {
    ...
    if (i_Rotation == 0.0) {
        vec2 pos = (i_Pos + (i_Quad_Vertex * i_Atlas_Size)) * u_Scale;
        gl_Position = vec4(pos.x - 1.0, pos.y + 1.0, 0.0, 1.0);
    } else {
        vec2 rotated_offset = rotate_around(i_Quad_Vertex * i_Atlas_Size, i_Rotate_Origin, i_Rotation);
