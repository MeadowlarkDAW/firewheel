use crate::{
    primitive::{self, Vertex2D},
    settings, Color, Point, Rect, Size,
};
use glam::Mat4;
use std::fmt::Debug;
use std::mem;
...
impl Pipeline {
    pub fn new(
        ...
        let uniforms_layout =
            device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                label: Some("goldenrod::triangle uniforms layout"),
        ...
        let uniforms_buffer = Buffer::new(
            "goldenrod::triangle uniforms buffer",
            device,
            UNIFORM_BUFFER_SIZE,
        ...
        let uniforms_bind_group =
            device.create_bind_group(&wgpu::BindGroupDescriptor {
                label: Some("goldenrod::triangle uniforms bind group"),
        ...
        let pipeline_layout =
            device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                label: Some("goldenrod::triangle pipeline layout"),
        ...
        let pipeline =
            device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
                label: Some("goldenrod::triangle pipeline"),
        ...
        let vertex_buffer = Buffer::new(
            "goldenrod::triangle vertex buffer",
            device,
            VERTEX_BUFFER_SIZE,
        ...
        let index_buffer = Buffer::new(
            "goldenrod::triangle index buffer",
            device,
            INDEX_BUFFER_SIZE,
        ...
        let blit =
            antialiasing.map(|a| msaa::Blit::new(device, texture_format, a));
    ...
    pub fn render(
        ...
        for mesh in meshes {
            ...
            let vertices = mesh.buffers.vertices.as_bytes();
            let indices = mesh.buffers.indices.as_bytes();
...

        {
                ...
                offsets.into_iter().enumerate()
            {
                render_pass.set_scissor_rect(
                    meshes[i].clip_bounds.top_left.x.round() as u32,
                    meshes[i].clip_bounds.top_left.y.round() as u32,
                    meshes[i].clip_bounds.size.width().round() as u32,
                    meshes[i].clip_bounds.size.height().round() as u32,
                );
...
impl From<Mat4> for Uniforms {
    fn from(transformation: Mat4) -> Uniforms {
        Self {
            transform: transformation.to_cols_array(),
            _padding_a: [0.0; 32],
            _padding_b: [0.0; 16],
        }
    }
...
pub struct Instance<'a> {
    ...
    pub buffers: &'a primitive::Mesh2D,
